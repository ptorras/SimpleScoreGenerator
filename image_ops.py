...
def _generate_rough_granular_texture_compat(self, oxsize, oysize):
    ...
    for _ in range(50):
        intensity = np.random.randint(1, 2)
    ...
    wave_grid_output = cv2.resize(
        wave_grid_output, (oxsize, oysize), interpolation=cv2.INTER_LINEAR
    )

    return wave_grid_output


def _generate_dot_granular_texture_compat(self, oxsize, oysize):
    ysize, xsize = 1000, 1000
...
TextureGenerator.generate_rough_granular_texture = (
    _generate_rough_granular_texture_compat
)
TextureGenerator.generate_dot_granular_texture = _generate_dot_granular_texture_compat


class CustomPaperFactory(Augmentation):
    def __init__(
        self,
        ...
        self.paper_textures = [
            np.array(Image.open(background_path))
            for background_path in self.background_path.glob("*")
        ]

    def retrieve_texture(self, image: np.ndarray):
        ...
        if (texture.shape[0] > shape[0]) and (texture.shape[1] > shape[1]):
            ...
            texture = texture[
                start_y : start_y + shape[0], start_x : start_x + shape[1]
            ]
...
class PaperFactory(Augmentation):
    ...
    def check_paper_edges(self, texture):
        ...
        _, texture_binary = cv2.threshold(
            texture_gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU
        )
        ...
        center_average = np.average(
            texture_blur[center_y - 10 : center_y + 10, center_x - 10 : center_x + 10]
        )
        ...
        # erode
        texture_binary = cv2.erode(
            texture_binary, np.ones((9, 9), np.uint8), iterations=1
        )

        # find contours in image
        contours, hierarchy = cv2.findContours(
            texture_binary, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE
        )
        ...
        if len(areas) > 0:
            ...
            for i, area_index in enumerate(area_indexs):
                ...
                if i == len(area_indexs) - 1:
                    ...
                    if (
                        areas[area_indexs[i]] >= min_area
                        and areas[area_indexs[i + 1]] < min_area
                    ):
                        max_contour = contours[area_index]
                        break
                    else:
            ...
            bbox = np.intp(cv2.boxPoints(rectangle))
    ...
    def __call__(
        ...
        if force or self.should_run():
            ...
            if texture_enable_color:
                if len(texture.shape) < 3:
                    ...
                    color_hsv = cv2.cvtColor(
                        np.array([[color]], dtype="uint8"), cv2.COLOR_BGR2HSV
                    )
                    ...
                    color_paper = ColorPaper(
                        hue_range=hue_range, saturation_range=saturation_range
                    )
                    ...
                    image_color = np.full(
                        (texture.shape[0], texture.shape[1], 3),
                        fill_value=color,
                        dtype="uint8",
                    )
                    ob = OverlayBuilder(
            ...
            if texture_intensity < target_intensity:
                brighten_ratio = (
                    abs(texture_intensity - target_intensity) / texture_intensity
                )
                brighten_min = 1 + (brighten_ratio / 2)
                brighten_max = 1 + brighten_ratio
                brightness = Brightness(
                    brightness_range=(brighten_min, brighten_max), min_brightness=1
                )
